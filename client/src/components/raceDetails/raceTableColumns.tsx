  ...
  return [
    ...
    },
    {
      ...
      cell: (cellData: { row: { original: Race } }) => {
        ...
        return (
          <p
            key={raceInfo.name}
            className="py-2 font-bold text-slate-700 tracking-wide"
          >
            {raceInfo.name}
          </p>
        );
    ...
    },
    {
      ...
      cell: (cellData: { row: { original: Race } }) => {
        ...
        return (
          <p
            className="font-bold text-slate-700 tracking-wide"
            key={raceInfo.startTime.toString()}
          >
            {new Date(raceInfo.startTime).toLocaleString()}
          </p>
        );
      },
    },
