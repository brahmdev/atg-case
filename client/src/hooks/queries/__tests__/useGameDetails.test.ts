...
describe("useGameDetails", () => {
  ...
  it("returns response with a successful request", async () => {
    ...
    const { result } = renderHook(() => useGameDetailsQuery({ gameId: "V75_2023-05-27_5_5" }), { wrapper: hookWrapper });
  ...
  it("returns error response when request is rejected", async () => {
     ...
     */
    const { result } = renderHook(() => useGameDetailsQuery({ gameId: "V86_2023-05-27_5_5" }), { wrapper: hookWrapper });
